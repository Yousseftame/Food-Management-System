  ...
  export default function verifyAccount() {
    ...
    const onSubmit = async(data)=> {
     ...
     //  let response = await axiosInstance.post(USERS_URLS.FORGET_PASS, data);
      ...
      navigate('/Login' ,{state:data.email} ); // uselocation
          ...
          <div className="row  vh-100 align-items-center justify-content-center">
            <div className="col-md-6 bg-white rounded-3 p-5">
              ...
              <div className="title my-3">
                ...
                <span className='text-muted'>No worries! Please enter your email and Code  </span>
              </div>
              <form onSubmit={handleSubmit(onSubmit)} className=' mt-4 ' >
                ...
                })} type="text" disabled className="form-control" placeholder="Email" aria-label="Username" aria-describedby="addon-wrapping"/>
             ...
             <input  placeholder='OTP' className="form-control" type='text' {...register("code",{required:"OTP is required"})}/>
              </div>
              {errors.code?.type=="required"&&<span className='text-danger ps-1'>{errors.code.message}</span>}
 
  
               
  
                <button  disabled={isSubmitting}  className='auth-button btn btn-success w-100 mt-3'>  {isSubmitting? "Submitting ...":"Verify Account "} </button>
